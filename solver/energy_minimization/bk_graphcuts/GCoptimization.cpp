...
template <>
void GCoptimization::setupDataCostsExpansion<GCoptimization::DataCostFnSparse>(SiteID size,LabelID alpha_label,Energy_ *e,SiteID *activeSites)
{
...
template <>
void GCoptimization::applyNewLabeling<GCoptimization::DataCostFnSparse>(Energy_ *e,SiteID *activeSites,SiteID size,LabelID alpha_label)
{
	DataCostFnSparse* dc = (DataCostFnSparse*)m_datacostFn;
	DataCostFnSparse::iterator dciter = dc->begin(alpha_label);
	for ( SiteID i = 0; i < size; i++ )
...
OLGA_INLINE void GCoptimization::addterm1_checked(Energy_* e, VarID i, EnergyTermType e0, EnergyTermType e1)
{
...
OLGA_INLINE void GCoptimization::addterm1_checked(Energy_* e, VarID i, EnergyTermType e0, EnergyTermType e1, EnergyTermType w)
{
	if ( e0 > GCO_MAX_ENERGYTERM || e1 > GCO_MAX_ENERGYTERM )
...
OLGA_INLINE void GCoptimization::addterm2_checked(Energy_* e, VarID i, VarID j, EnergyTermType e00, EnergyTermType e01, EnergyTermType e10, EnergyTermType e11, EnergyTermType w)
{
...
template <typename DataCostT>
void GCoptimization::setupDataCostsExpansion(SiteID size,LabelID alpha_label,Energy_ *e,SiteID *activeSites)
{
...
template <typename SmoothCostT>
void GCoptimization::setupSmoothCostsExpansion(SiteID size,LabelID alpha_label,Energy_ *e,SiteID *activeSites)
{
...
template <typename DataCostT>
void GCoptimization::applyNewLabeling(Energy_ *e,SiteID *activeSites,SiteID size,LabelID alpha_label)
{
	DataCostT* dc = (DataCostT*)m_datacostFn;
	for ( SiteID i = 0; i < size; i++ )
	{
...
GCoptimization::EnergyType GCoptimization::setupLabelCostsExpansion(SiteID size,LabelID alpha_label,Energy_ *e,SiteID *activeSites)
{
...
bool GCoptimization::alpha_expansion(LabelID alpha_label)
{
	...
	try 
	{
		...
		Energy_ e(size+m_labelcostCount, // poor guess at number of pairwise terms needed :(
...
void GCoptimization::alpha_beta_swap(LabelID alpha_label, LabelID beta_label)
{
	...
	try
	{
		...
		// Create binary variables for each remaining site, add the data costs,
		// and compute the smooth costs between variables.
		Energy_ e(size,m_numNeighborsTotal,(void(*)(const char*))handleError);
		e.add_variable(size);
